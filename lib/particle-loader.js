...
ParticleLoader.prototype.genParticlePaths = function(particle,type,particlePath) {
  ...
  console.log(path);
    var pathObj = parsePath(path.join(particle.modulePath,'particles',this.typeMap[type],particlePath));
    var realPath = path.format(pathObj);

    var relativePathObj = parsePath(path.relative(path.resolve(process.cwd(),this.root),realPath));

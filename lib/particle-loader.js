...
ParticleLoader.prototype.loadParticle = function(type,cModule,particlePath,options) {
  var opts = _.merge({
    parentFile: null,
    cModule: ''
  ...
  var done = false;
  while (done === false) {
    particle.pathObj = path.parse(path.join(particle.modulePath,'particles',this.typeMap[type],particlePath));
    particle.path = path.format(particle.pathObj);

    particle.relativePathObj = path.parse(path.relative(path.resolve(process.cwd(),this.root),particle.path));
    particle.relativePath = path.format(particle.relativePathObj);

    particle.urlPath = path.posix.format(particle.relativePathObj);

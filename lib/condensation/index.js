 ...
 * @param {Array} options.s3 - list of s3 buckets/paths
 * @param {String} [dist="dist"] - top level distribution path name
 * @param {String} [root="./"] - root of the condensation project
 * @param {String} [taskPrefix="condensation"] - A prefix to add to all task names
 */
var Condensation = function Condensation(gulp,options) {
...
Condensation.prototype.condense = function condense() {
  ...
  _.each(s3config, function(s3opts,i) {
    ...
    gulp.task(
      ...
      [
        self.genTaskName('particle-modules:load'),
        self.genTaskName('clean'),
  ...
  gulp.task(self.genTaskName('particle-modules:load'), function() {
    ...
    .pipe(through.obj(function(file,enc,cb) {
      ...
      if (condensationJs && condensationJs.initialize) {
        if (condensationJs.initialize.length === 1) {
          condensationJs.initialize.apply(self,[cb]);
        }
        else {
          condensationJs.initialize.apply(
            self,
            [{options: self.options},cb]
          )
        }
      }
      else {

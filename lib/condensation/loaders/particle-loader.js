...
ParticleLoader.prototype.loadParticle = function(type,cModule,pPath,options) {
  ...
  if (!_.isString(pPath)) {
    throw new VError(loadError, '%s particlePath in "%s" must be a string', type, opts.parentFile.path);
  }
  ...
  if (!particle.fsPath) {
    throw new VError(loadError, '%s %s refrenced in %s could not be found', type, particlePath, opts.parentFile.path);
  }

  particle.format = cUtil.detectFormat({
    particle: particle
  });

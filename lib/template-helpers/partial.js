...
var helper = function(cModule,pPath,hArgs,hOpts,cOpts) {
    ...
    var fn = engine.compile(m.content);
    ...
    var tempalteContent;
    if (extended === true) {
      // If extending another partial of the same type merge options in reverse
    ...
    //return new engine.SafeString(fn(_.merge(_.merge(_.merge(m.data,hOpts.data.root),this),hOpts.hash),{data:data}));
